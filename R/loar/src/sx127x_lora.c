...
{
    HAL_SPI_Transmit(&hspi1, &send_data, 1, 0xffff);
}
...
{
    if(value)
     HAL_GPIO_WritePin(GPIOA, GPIO_PIN_11, GPIO_PIN_SET); //??λ?????????????   
    else
     HAL_GPIO_WritePin(GPIOA, GPIO_PIN_11, GPIO_PIN_RESET); //??λ?????????????
}

void zm4xx_sel_pin_set(uint8_t value)
{
    if(value)
     HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET); //??λ?????????????
    else
     HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET); //??λ?????????????
}

void timer0_16_delay_ms(uint32_t ms)
{
  HAL_Delay(ms);
}

void timer0_16_delay_us(uint32_t us)
{
